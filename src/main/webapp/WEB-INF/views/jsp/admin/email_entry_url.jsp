...
<body>
    <div id="wrap">
      ...
      <div class="container">
        ...
        <font style="text-align: left;" size="3" ><strong> Approved </strong> (<a href="<%=request.getContextPath() %>/emailEntryVerificationLog">${userVerificationApproved }</a>) </font> &nbsp;&nbsp; 
        <font style="text-align: left;" size="3" ><strong> Missed </strong> (<a href="<%=request.getContextPath() %>/emailEntryVerificationMissed">${userVerificationAll -(userVerificationActive +userVerificationApproved)}</a>)</font> &nbsp;&nbsp; &nbsp;&nbsp;   
      
		<c:if test="${userVerificationActive == 0}">
        	<input type="button" name="getMoreLinks" id="getMoreLinks" class="btn btn-primary" onclick="getMoreLinks('assignEmailEntry')" value="Get More 1 Links">
        </c:if><br />   
         ...
         <table class="table table-striped" >
             ...
             <c:forEach items="${urlList }" var="urlList" varStatus="index">         
	         	<tr>
	         		<td>${index.count } </td> 
	         		<td>      
	         			<a id="btnShowPopup" href="#" onclick="setEmailData('${urlList.urlId}')" > Content </a>
	         		</td> 
	         		<td><c:out value="${urlList.fromEmail}"></c:out></td>
    ...
    <div class="col-md-4">
     ...
     <br /> 
     <form action="insertEmailEntry" method="post">
...
<script type="text/javascript">
    function setEmailData(urlId){
    	window.open("getEmailDataByUrlId?urlId="+ urlId,'popUpWindow','left=50,top=50,resizable=yes,scrollbars=yes,toolbar=yes,menubar=no,location=no,directories=no, status=yes');
    }
